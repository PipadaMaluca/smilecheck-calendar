...
export function DesktopCalendarSidebar({
  ...
  return <aside className="h-full w-[200px] bg-[#0D2137] border-l border-[#1E3A5F] flex flex-col overflow-hidden flex-shrink-0">
      {/* Find Slot Button */}
      ...
      <div className="px-3 pb-3 border-b border-[#1E3A5F] flex-shrink-0">
        ...
        <div className="flex items-center justify-between mb-2">
          <Button variant="ghost" size="icon" onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="h-6 w-6 text-muted-foreground hover:text-foreground hover:bg-[#152238]">
          ...
          <Button variant="ghost" size="icon" onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="h-6 w-6 text-muted-foreground hover:text-foreground hover:bg-[#152238]">
        ...
        <div className="grid grid-cols-7 gap-0.5 mb-1">
          {weekDays.map((d, i) => <div key={`${d}-${i}`} className="text-center text-[10px] font-medium text-muted-foreground py-0.5">
              {d}
            </div>)}
        </div>
        ...
        <div className="grid grid-cols-7 gap-0.5">
          ...
          return <button key={idx} onClick={() => onDateSelect(date)} className={cn('relative flex flex-col items-center justify-center w-6 h-6 rounded text-[10px] transition-all', !isCurrentMonth && 'text-muted-foreground/40', isCurrentMonth && 'text-foreground hover:bg-[#152238]', isToday && 'bg-primary text-primary-foreground font-semibold', isSelected && !isToday && 'bg-primary/20 text-primary ring-1 ring-primary')}>
                {format(date, 'd')}
                {hasAppt && !isToday && <span className="absolute bottom-0 w-1 h-1 rounded-full bg-primary" />}
              </button>;
        })}
        </div>
      </div>
      ...
      <div className="p-3 border-b border-[#1E3A5F] flex-shrink-0">
        ...
        <Button variant="secondary" size="sm" className="w-full text-xs font-medium bg-primary/20 hover:bg-primary/30 text-primary" onClick={onSelectAllDentists}>
          FILTRAR PRESENTES
        </Button>
      </div>
      ...
      <div className="px-3 py-2 flex-shrink-0">
        <div className="relative">
          ...
          <Input placeholder="Pesquisar" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-7 h-7 text-xs bg-[#152238] border-[#1E3A5F]" />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3 scrollbar-hide">
        {/* All checkbox */}
        <div className="flex items-center gap-2 py-1.5 cursor-pointer hover:bg-[#152238] rounded px-1.5 -mx-1.5" onClick={onSelectAllDentists}>
          <Checkbox checked={allSelected} className="border-muted-foreground h-3.5 w-3.5 data-[state=checked]:bg-primary data-[state=checked]:border-primary" />
        ...
        <Collapsible open={isProfessionalsOpen} onOpenChange={setIsProfessionalsOpen}>
          ...
          <CollapsibleContent className="pl-3">
            ...
            return <div key={dentist.id} className={cn("flex items-center gap-2 py-1 cursor-pointer hover:bg-[#152238] rounded px-1.5 -mx-1.5", isSelf && "opacity-90")} onClick={() => !isSelf && onDentistToggle(dentist.id)}>
                  <Checkbox checked={isSelected} disabled={isSelf} className="border-muted-foreground h-3.5 w-3.5 data-[state=checked]:bg-primary data-[state=checked]:border-primary" />
        ...
        {/* Add Dentist */}
        <Button variant="ghost" size="sm" className="w-full justify-start gap-1.5 text-primary text-xs mt-1 h-7 hover:bg-[#152238]">
