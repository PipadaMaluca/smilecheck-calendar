...
const CONSULTATION_COLORS: Record<string, {
  bg: string;
  border: string;
  text: string;
}> = {
      ...
      <div ref={timelineRef} className="flex-1 overflow-y-auto overflow-x-auto scrollbar-hide">
        ...
        height: `${hours.length * HOUR_HEIGHT}px`
          ...
          <div className="absolute top-0 left-16 right-0 flex h-full">
            ...
            return <div key={dentist.id} className={cn("flex-1 relative border-l border-primary", colIdx === 0 && 'border-l-0')}>
                  {/* Blocked Slots */}
                ...
                return <div key={consultation.id} onClick={() => onSlotClick(slot)} className={cn('absolute left-1 right-1 rounded-md p-2 cursor-pointer transition-all', 'hover:scale-[1.02] hover:shadow-lg border-l-4', colors.bg, colors.border)} style={{
                  top: `${getSlotPosition(slot.time)}px`,
                        ...
                        <div className="flex items-start justify-between gap-1">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              ...
                              {isTeleconsulta && <Video className={cn("w-3 h-3", colors.text)} />}
                              {isUrgent && <AlertTriangle className="w-3 h-3 text-urgencia" />}
                            </div>
                          ...
                          <div className="flex-shrink-0">
                            {consultation.isPaid ? <Check className="w-3 h-3 text-primary" /> : <Flag className="w-3 h-3 text-primeira-consulta" />}
                          </div>
                        </div>
                      </div>;
              })}
