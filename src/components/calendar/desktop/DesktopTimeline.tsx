...
export function DesktopTimeline({
  ...
  return (
    <div className="flex-1 flex flex-col bg-background overflow-hidden">
      ...
      <div
        ref={timelineRef}
        className="flex-1 overflow-y-auto overflow-x-auto scrollbar-hide"
      >
        <div
          className="relative"
          ...
          <div className="absolute top-0 left-16 right-0 flex h-full">
            {dentists.map((dentist, colIdx) => {
              ...
              const occupiedSlots = slots.filter(
                (s) => s.status === 'ocupado' && s.consultation
              );
              ...
              return (
                ...
                >
                  ...
                  {blockedSlots.map((slot, slotIdx) => (
                    <div
                      key={`blocked-${slotIdx}`}
                      className="absolute left-1 right-1 bg-[#9E9E9E]/30 rounded flex items-center justify-center border-l-4 border-l-[#9E9E9E]"
                      style={{
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              ...
                              {(isTeleconsulta || isTeleconsultaUrgente) && (
                                <Video className="w-3 h-3" style={{ color: styles.borderColor }} />
                              )}
                              {isUrgent && (
                                <AlertTriangle className="w-3 h-3 text-[#F44336]" />
                              )}
                            </div>
                            <p className="text-xs font-bold uppercase truncate mt-0.5 text-white">
                              {consultation.patient.name}
                            </p>
                            <p className="text-[10px] truncate" style={{ color: styles.borderColor }}>
