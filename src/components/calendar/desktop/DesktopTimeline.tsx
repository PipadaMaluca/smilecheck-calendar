...
export function DesktopTimeline({
  ...
  return <div className="flex-1 flex flex-col bg-[#1A2F3D] overflow-hidden">
      {/* Dentist Headers */}
      <div className="flex border-b border-border bg-card/50 sticky top-0 z-10">
        ...
        {dentists.map(dentist => <div key={dentist.id} className="flex-1 px-3 py-2 border-l border-border first:border-l-0">
      ...
      <div ref={timelineRef} className="flex-1 overflow-y-auto overflow-x-auto scrollbar-hide">
        ...
        minHeight: `${hours.length * HOUR_HEIGHT}px`
          ...
          <div className="w-16 flex-shrink-0 relative border-r border-[#1E3A5F]">
            {hours.map((hour, idx) => <div key={hour} className="absolute left-0 right-0 flex items-start justify-end pr-2 pt-0.5" style={{
          ...
          <div className="flex-1 flex relative">
            ...
            <div className="absolute inset-0 pointer-events-none">
              {hours.map((hour, idx) => <div key={hour} className="absolute left-0 right-0 border-t border-border/50" style={{
              top: `${idx * HOUR_HEIGHT}px`
            }} />)}
              {/* Half-hour lines */}
              {hours.slice(0, -1).map((hour, idx) => <div key={`half-${hour}`} className="absolute left-0 right-0 border-t border-border/20" style={{
              top: `${idx * HOUR_HEIGHT + SLOT_HEIGHT}px`
            }} />)}
            </div>

            {/* Current Time Line */}
            {currentTimePosition !== null && <div className="absolute left-0 right-0 z-20 flex items-center pointer-events-none" style={{
            ...
            return <div key={dentist.id} className={cn("flex-1 relative bg-slate-700", colIdx > 0 && 'border-l border-border')} style={{
              minHeight: `${hours.length * HOUR_HEIGHT}px`
                  ...
                  {/* Blocked Slots */}
                  {blockedSlots.map((slot, slotIdx) => <div key={`blocked-${slotIdx}`} className="absolute left-0.5 right-0.5 bg-[#9E9E9E]/30 rounded flex items-center justify-center border-l-2 border-l-[#9E9E9E]" style={{
                ...
                return <div key={consultation.id} onClick={() => onSlotClick(slot)} className={cn('absolute left-0.5 right-0.5 rounded cursor-pointer transition-all', 'hover:scale-[1.02] hover:shadow-lg border-l-3')} style={{
                  top: `${getSlotPosition(slot.time)}px`,
                  ...
                  backgroundColor: `${styles.borderColor}30`
                        ...
                        <div className="flex items-start justify-between gap-0.5 p-1 h-full overflow-hidden">
                          ...
                          <div className="flex-shrink-0">
                            {consultation.isPaid ? <Check className="w-2.5 h-2.5 text-primary" /> : <Flag className="w-2.5 h-2.5 text-[#FDD835]" />}
                          </div>
                        </div>
                      </div>;
              })}
