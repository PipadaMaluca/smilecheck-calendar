...
export function PasteConfirmationModal({
  consultation,
  targetDate,
  targetTime,
  targetDentistName,
  ...
  const handleConfirm = () => {
    ...
    toast.success('Consulta colada com sucesso!');
    onClose();
  };
  ...
  return (
    <>
      ...
      <div className={cn(
        'bg-card flex flex-col z-[61]',
        isMobile
          ? 'fixed inset-x-0 bottom-0 rounded-t-2xl max-h-[90vh] animate-slide-up-modal'
          : 'fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-2xl w-[95%] max-w-lg max-h-[90vh]'
      )}>
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h2 className="text-lg font-bold">Confirmar Colagem</h2>
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 space-y-2">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-medium">
                {format(targetDate, "EEEE, d 'de' MMMM yyyy", { locale: pt })}
              </span>
            </div>
            <div className="flex items-center gap-2 text-sm">
            ...
            {targetDentistName && (
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">{targetDentistName}</span>
              </div>
            )}
          </div>
