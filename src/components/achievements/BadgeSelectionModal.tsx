...
export function BadgeSelectionModal({ open, onOpenChange, categories, selectedIds, onSave }: BadgeSelectionModalProps) {
  ...
  const allAchievements = useMemo(() =>
    categories.flatMap(c => c.achievements),
    [categories]
  );
  ...
  const toggle = (id: string) => {
    ...
    if (selected.includes(id)) {
      ...
      if (selected.length >= 8) {
        toast.info('Remova uma conquista primeiro (máximo 8)');
        return;
      }
  ...
  const selectedAchievements = selected
      ...
      <DialogContent className={cn('max-h-[90vh] flex flex-col', isMobile ? 'max-w-[95vw]' : 'max-w-2xl')}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Escolher Conquistas em Destaque
          </DialogTitle>
          <DialogDescription>
            Selecione até 8 conquistas para mostrar no seu perfil
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="bg-secondary/30 rounded-xl p-3 border border-border">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={cn(
              ...
              selected.length === 8 ? 'bg-primary/20 text-primary' : 'bg-muted text-muted-foreground'
            )}>
              {selected.length}/8 selecionadas
            </span>
          </div>
            ...
            {Array.from({ length: 8 }).map((_, i) => {
              ...
              return (
                <div key={i} className={cn(
                  ...
                  ach
                    ? 'bg-gradient-to-br ' + BADGE_TIER_STYLES[getBadgeTier(ach)].bgGradient + ' border ' + BADGE_TIER_STYLES[getBadgeTier(ach)].borderColor
        ...
        <DialogFooter className="flex gap-2 sm:gap-2">
          ...
          <Button onClick={() => { onSave(selected); handleOpenChange(false); toast.success('Conquistas em destaque atualizadas!'); }}>
            Guardar
          </Button>
        </DialogFooter>
      </DialogContent>
