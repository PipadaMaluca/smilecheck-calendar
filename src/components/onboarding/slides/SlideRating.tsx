...
const ratingLegend = [
  { stars: '1-2★', label: 'Problemático', color: '#F44336' },
  { stars: '3★', label: 'Atenção', color: '#FF9800' },
  { stars: '4-5★', label: 'Bom/Excelente', color: '#4CAF50' },
  { stars: '5★', label: 'Destaque especial!', color: '#FFD700', special: true },
];

export const SlideRating = ({ isActive, userRole }: SlideRatingProps) => {
  const isClinic = userRole === 'clinic';
  const title = isClinic ? '⭐ A SUA CLASSIFICAÇÃO' : '⭐ A TUA CLASSIFICAÇÃO';
  const bottomText = isClinic
    ? 'Mantenha boa classificação para ser prioritária nas pesquisas!'
    : 'Mantém boa classificação para seres prioritário nas pesquisas!';
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center px-6">
      ...
      <div className={`glass-card-strong p-8 mb-8 glow-green ${isActive ? 'animate-pulse-glow' : ''}`}>
        ...
        <div className="flex gap-1 mt-2 justify-center">
          {[1, 2, 3, 4, 5].map((star) => (
            <Star
              key={star}
              className={`w-5 h-5 ${star <= 4 ? 'fill-current' : 'opacity-50'}`}
      ...
      <div className="glass-card p-5 w-full max-w-sm space-y-3">
        <p className="text-muted-foreground text-sm text-center mb-4">Legenda de classificação:</p>
          ...
          >
            <div 
              ...
              style={{ 
                ...
                boxShadow: item.special ? `0 0 8px ${item.color}` : 'none'
              }} 
            />
            ...
            <span className="font-medium ml-auto flex items-center" style={{ color: item.color }}>
              {item.label}
              {item.special && (
                <Sparkles className="inline-block w-4 h-4 ml-1 animate-sparkle" style={{ color: item.color }} />
              )}
            </span>
          </div>
        ))}
      ...
      <p className="mt-6 text-muted-foreground text-center text-sm max-w-xs">
        {bottomText.split('prioritári')[0]}
        <span className="text-gaming-diamond font-semibold underline">prioritári{isClinic ? 'a' : 'o'}</span> nas pesquisas!
      </p>
    </div>
  );
};
