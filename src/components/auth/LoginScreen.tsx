...
export function LoginScreen() {
  ...
  const loginForm = (
    <div className="flex flex-col items-center animate-fade-in">
      ...
      <p className="text-muted-foreground mb-6 text-lg">O seu sorriso, a nossa prioridade</p>
      ...
      <p className="mt-6 text-muted-foreground text-base">
        Não tem conta?{' '}
        <button onClick={() => navigate('/signup')} className="text-primary hover:underline font-medium">Criar conta</button>
      </p>
  ...
  const qrSection = (
    <div className="flex flex-col items-center">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-xl font-bold text-foreground">Entrar com QR Code</h2>
      </div>
      <p className="text-muted-foreground mb-6 text-sm">Rápido e sem password</p>
      <QRCodeDisplay onAuthorized={handleQRAuthorized} />
    </div>
  );

  // Mobile: stacked with collapsible QR
  if (isMobile) {
    return (
      <AuthBackground>
        <div className="space-y-6">
          ...
          <div className="border-t border-border pt-4">
            ...
            >
              ...
              <span>Ou entre com QR Code</span>
  ...
  return (
    <div className="min-h-screen bg-background flex relative overflow-hidden">
      ...
      {/* Right Half — QR Code Login */}
      <div className="w-1/2 md:w-[45%] lg:w-1/2 flex items-center justify-center p-8 z-10 relative">
        <div className="w-full max-w-sm bg-card/30 backdrop-blur-sm rounded-2xl p-8 border border-border/50">
          {qrSection}
        </div>
      </div>
    </div>
