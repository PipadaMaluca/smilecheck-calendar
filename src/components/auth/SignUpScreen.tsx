...
export function SignUpScreen() {
  ...
  const validateStep2 = () => {
    ...
    if (!phoneDigits) e.phone = 'Número de telemóvel é obrigatório';
    else if (phoneDigits.length < 9) e.phone = 'Mínimo 9 dígitos';
  ...
  const handleSocialLogin = (provider: string) => {
    ...
    // Simulate social auth providing name and email
    ...
    setStep(4); // Complete profile step
  };
  ...
  const validateSocialProfile = () => {
    ...
    if (!password) e.password = 'Password de backup é obrigatória';
    else if (password.length < 6) e.password = 'Mínimo 6 caracteres';
    if (password !== confirmPassword) e.confirmPassword = 'Passwords não coincidem';
    if (!acceptedTerms) e.terms = 'Deve aceitar os termos';
    if (accountType === 'dentista' && !orderNumber.trim()) e.orderNumber = 'Número da Ordem é obrigatório';
  ...
  const handleCompleteSocialProfile = async () => {
    ...
    await new Promise(r => setTimeout(r, 1000));
    setLoading(false);
    // Skip email verification, go directly to SMS
    setVerificationPhase('sms');
    setSmsOtpValue('');
    setStep(3);
    startSmsResendTimer();
    ...
    return (
      <AuthBackground>
        <div className="flex flex-col items-center animate-fade-in">
          ...
          <h1 className="text-xl font-bold text-foreground mb-1">Verifique o seu telemóvel</h1>
          <p className="text-sm text-muted-foreground text-center mb-1">
            Enviámos um SMS para <span className="text-foreground font-medium">{maskPhone(countryCode, phone)}</span>
          </p>
          ...
          <Button onClick={handleVerifySms} disabled={smsOtpValue.length < 6 || loading} className="w-full h-12 text-base font-semibold mb-4">
            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Verificar Telemóvel'}
          </Button>

          <div className="flex gap-4 text-sm">
            <button
              onClick={() => { if (smsResendCountdown === 0) startSmsResendTimer(); }}
              className={cn('text-primary hover:underline', smsResendCountdown > 0 && 'text-muted-foreground pointer-events-none')}
            >
              {smsResendCountdown > 0 ? `Reenviar SMS (${smsResendCountdown}s)` : 'Reenviar SMS'}
            </button>
          </div>
        </div>
      </AuthBackground>
    ...
    return (
      <AuthBackground>
        <div className="flex flex-col animate-fade-in">
          ...
          <h1 className="text-xl font-bold text-foreground mb-1 text-center">Completar Registo</h1>
          <p className="text-sm text-muted-foreground text-center mb-6">
            Quase lá! Confirme os seus dados e preencha os campos em falta.
          </p>

          <div className="w-full space-y-3">
            {/* Pre-filled from social — with green checkmark */}
            ...
            <div className="relative">
              <Input type="email" placeholder="Email" value={email} onChange={e => { setEmail(e.target.value); setErrors(p => ({ ...p, email: '' })); }}
            ...
            {/* Required fields — blue left border */}
            ...
            <div className="border-l-2 border-primary pl-3">
              <div className="relative">
                <Input type={showPassword ? 'text' : 'password'} placeholder="Password de backup" value={password}
            ...
            <div className="border-l-2 border-primary pl-3">
              <Input type={showPassword ? 'text' : 'password'} placeholder="Confirmar password de backup" value={confirmPassword}
            ...
            <p className="text-xs text-muted-foreground px-3">
              🔒 Esta password permite aceder à sua conta mesmo sem {socialProvider}. Recomendamos uma password diferente.
            </p>

            {/* Account-type specific fields */}
            {accountType === 'dentista' && (
              <div className="border-l-2 border-primary pl-3">
                <Input placeholder="Nº Ordem dos Médicos Dentistas" value={orderNumber}
            ...
            <Button onClick={handleCompleteSocialProfile} disabled={loading} className="w-full h-12 text-base font-semibold mt-2">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Concluir Registo'}
            </Button>
          </div>
        </div>
      </AuthBackground>
