...
export function ClinicProfileView({ clinicId, isOpen, onClose, onViewDentistProfile, inline, isFavorite, onToggleFavorite, isOwnProfile, onEditProfile }: ClinicProfileViewProps) {
  ...
  const realRating = useRealRatingForMockId('clinic', clinicId);
  const hasRealRating = realRating.count > 0;
  const displayRating = hasRealRating && realRating.average !== null ? realRating.average : data.rating;
  const displayReviewCount = hasRealRating ? realRating.count : data.reviewCount;
  const reviews = hasRealRating
    ...
    : (CLINIC_REVIEWS[clinicId] || CLINIC_REVIEWS['1']);
...
<Glyph emoji="♿" className="inline w-3.5 h-3.5 mr-1 -mt-0.5" />{a}</Badge>
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: t('profile.totalConsultations'), value: stats.totalConsultations, icon: Stethoscope, trendKey: 'totalConsultations' },
            { label: t('profile.activePatients'), value: stats.activePatients, icon: Users, trendKey: 'activePatients' },
            { label: t('profile.confirmationRate'), value: stats.confirmationRate, icon: TrendingUp, trendKey: 'confirmationRate' },
            { label: t('profile.activeDentists'), value: String(stats.activeDentists), icon: User, trendKey: 'activeDentists' },
          ].map(stat => {
            const trend = CLINIC_TRENDS[stat.trendKey];
